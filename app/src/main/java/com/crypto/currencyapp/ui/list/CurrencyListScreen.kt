...
@Composable
fun CurrencyListScreen(uiState: UiState<List<CurrencyInfo>>) {
    Box(modifier = Modifier.fillMaxSize()) {
        when (uiState) {
            ...
            is UiState.Success -> {
                LazyColumn {
                    items(uiState.data, key = { it.id }) { currency ->
...
fun CurrencyListItem(currency: CurrencyInfo) {
    Row(
        ...
        Column(modifier = Modifier.weight(1f)) {
            Text(text = currency.name, fontWeight = FontWeight.Bold)
            Text(text = currency.symbol, style = MaterialTheme.typography.bodySmall)
        }
